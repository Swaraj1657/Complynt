...
function initLayout(activePage) {
    ...
    aside.innerHTML = `
        ...
        <div class="mt-auto pt-6 border-t border-primary-container/10">
            <button class="w-full primary-gradient text-white rounded-lg py-2.5 text-sm font-semibold flex items-center justify-center gap-2 shadow-lg shadow-primary/20 hover:opacity-90 active:scale-95 transition-all" onclick="window.location.href='/new-complaint.html'">
...
function statusBadge(s) {
    ...
    return `<span class="px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${colors[s] || 'bg-slate-100 text-slate-600'}">${s.replace(/_/g, ' ')}</span>`;
}
...
function severityBadge(s) {
    ...
    return `<span class="px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-tight ${colors[s] || 'bg-slate-200'}">${labels[s] || s}</span>`;
}
...
function slaBadge(sla) {
    ...
    return `<span class="px-2 py-0.5 rounded text-[10px] font-bold uppercase ${colors[sla.status] || 'bg-slate-100'}">${sla.status.replace(/_/g, ' ')}</span>`;
}
